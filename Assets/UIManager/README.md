...
> ### Prerequisites
> >
> > 1. **Addressables Setup**
> > UIManager uses Unity Addressables to load screen prefabs. Each screen prefab must be:
> >   - Added to an **Addressables Group** (Window → Asset Management → Addressables → Groups)
> >   - Assigned an **address** matching the pattern `{addressablePrefix}/{prefabName}`
> >
> >     Example: with `addressablePrefix = "Screens"` and `prefabName = "EmptyScreen"`,
> >     the Addressables address must be `"Screens/EmptyScreen"`.
> > 
> > 2. **UI Root Canvas**  
...
> >
> > `string prefabName`  
> > The name used to build the Addressables address: `"{addressablePrefix}/{prefabName}"`.
> > Also serves as the cache key for recycled screens.
> > 
