...
namespace BlitzyUI
{
    public class UIManager : MonoBehaviour
    {
        ...
        [Tooltip("Addressables address prefix for screen prefabs. " +
                 "The full address will be: \"{addressablePrefix}/{prefabName}\".\n" +
                 "Example: prefix \"Screens\" + prefabName \"EmptyScreen\" → address \"Screens/EmptyScreen\"")]
        public string addressablePrefix = "Screens";
        ...
        private void OnDestroy()
        {
            ...
            // Release all Addressables instances still in the stack.
            ...
            // Release all Addressables instances still in the cache.
        ...
        private async void ExecuteNextQueueItem()
        {
            ...
            if (queued is QueuedScreenPush)
            {
                ...
                {
                    // Set state immediately to prevent re-entry during async Addressables loading.
                    _state = State.Push;

                    // Build the Addressables address: "{prefix}/{prefabName}"
                    // Example: prefix "Screens" + prefabName "EmptyScreen" → "Screens/EmptyScreen"
                    string address = string.IsNullOrEmpty(addressablePrefix)
                        ? queuedPush.prefabName
                        : $"{addressablePrefix}/{queuedPush.prefabName}.prefab";

                    var handle = Addressables.InstantiateAsync(address, rootCanvas.transform);
                    await handle.Task;

                    if (handle.Status != AsyncOperationStatus.Succeeded)
                    {
                        Debug.LogError($"[UIManager] Failed to load Addressable: \"{address}\" (status: {handle.Status})");
                    ...
                    screenInstance = handle.Result.GetComponent<Screen>();
        ...
        private void HandlePopFinished(Screen screen)
        {
            ...
            {
                // Destroy screen and release Addressables instance.
                Addressables.ReleaseInstance(screen.gameObject);
            }
